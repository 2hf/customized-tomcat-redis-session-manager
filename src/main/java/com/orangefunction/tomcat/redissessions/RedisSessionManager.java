...
public class RedisSessionManager extends ManagerBase implements Lifecycle {
  ...
  public Session createSession(String requestedSessionId) {
    ...
    try {
      ...
      if (null != requestedSessionId) {
        ...
        if (jedis.setnx(this.getSessionRedisKey(sessionId).getBytes(), NULL_SESSION) == 0L) {
          sessionId = null;
        }
      } else {
        do {
        ...
        } while (jedis.setnx(this.getSessionRedisKey(sessionId).getBytes(), NULL_SESSION) == 0L); // 1 = key set; 0 = key already existed
      }
  ...
  public byte[] loadSessionDataFromRedis(String id) throws IOException {
    ...
    try {
      ...
      byte[] data = jedis.get(this.getSessionRedisKey(id).getBytes());
      error = false;
  ...
  protected boolean saveInternal(Jedis jedis, Session session, boolean forceSave) throws IOException {
    ...
    try {
      ...
      byte[] binaryId = this.getSessionRedisKey(redisSession.getId()).getBytes();
  ...
  public void remove(Session session, boolean update) {
    ...
    try {
      ...
      jedis.del(this.getSessionRedisKey(session.getId()));
      error = false;
    } finally {
      if (jedis != null) {
        returnConnection(jedis, error);
